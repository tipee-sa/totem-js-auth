...
define(['srp', 'jquery', 'otp', 'cryptojs'], function (Srp, jQuery, Otp, CryptoJS) {
    ...
    /**
     *  Corps JSON d'une réponse en erreur, ou un objet vide s'il n'est pas du JSON.
     *
     *  @param   {jqXHR} xhr
     *  @returns {Object}
     */
    ...
    session.prototype = {
        ...
        make_request: function (add_clear) {
            ...
            jQuery.ajax(
                ...
                // A non-JSON body (proxy error page) must not throw here, or the promise
                // never settles and the caller waits forever.
                var result = parseJsonBody(xhr);
                ...
                if (xhr.status === 422) {
                    ...
                    that.promise.reject('too_many_attempts');
                }
            ...
            var that = this,
                fn = function (data) {
                    if (data.sess_id !== undefined) {
                        that.session_success(data.sess_id, data.user_id, data.key);
        ...
        validateKey: function () {
            ...
            jQuery.post(
                ...
                // Same fallback as make_request: an unexpected status (5xx, aborted request)
                // must still settle the promise, or the caller waits forever.
                that.promise.reject('unknown');
            });
        },
